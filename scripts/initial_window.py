...
class InitialWindow(Gtk.Window):
    ...
    def setup_ui(self):
        ...
        vbox_main.pack_end(hbox_buttons, False, False, 0)
    ...
    def add_card(self, config):
        ...
        card_button.connect("clicked", self.server_details, config['Name'], ', '.join(config['IPv4']), ', '.join(config['IPv6']))

        self.vbox_cards.pack_start(card_button, False, False, 0)
        self.show_all()

    def server_details(self, widget, name, ipv4, ipv6):
